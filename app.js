...
(function () {
	...
	try {
		global.BotConfig = JSON.parse(fs.readFileSync('./config/Vanilla/config.json', 'utf8', function (err, data) {
	...
	if (!BotConfig) {
		fs.mkdirSync(`./config/Vanilla/`, { recursive: true });
		fs.writeFile(`./config/Vanilla/config.json`, JSON.stringify({
	...
	app.listen(port, () => {
		console.log(`VanillaPremium v${version} is listening on port ${port}!`);
	});
